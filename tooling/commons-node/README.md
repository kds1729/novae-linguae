...
| `GET /v0/witnesses` — countersigned PEER anchors (the federated half of anchoring: signature-verified, root-compared against this node's own replicated corpus, signed with its anchor identity; the `witness_anchors` task runs per peer at the START of each replication pass — ordered before the bulk sync so the peer's rate limit can't starve its one request, `manage.py witnessanchors <peer>` is the manual form) | ✅ |
