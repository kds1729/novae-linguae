...
WRITE_CONVENTIONS = (
    ...
    "- Other builtins, applied by juxtaposition: neg abs min max length append reverse cons head tail "
    "last init null map filter foldl foldr.\n"
    "- Floats: a float literal is a bare decimal WITH a fraction part: 0.0, 3.25 (a whole float is "
    "3.0, never 3). to_float widens an int to a float; arithmetic (+ - * / %) and to_string work over "
    "int OR float (both operands the same type — mixing needs to_float; to_string of a float is the "
    "canonical rendering: to_string 3.0 is \"3\"). Division by zero errors in both domains.\n"
    "- String builtins (pattern/separator argument FIRST): str_concat str_length str_contains str_split "
    "str_join to_string parse_int, plus str_lt (lexicographic code-point order, same order as sorted map "
    "keys), str_lower (Unicode default lowercase), and url_encode (RFC 3986 strict percent-encoding: "
    "unreserved characters pass, every other UTF-8 byte becomes %XX uppercase hex). String literals are "
    "double-quoted: \"a,b\". "
    "parse_int returns a Maybe (`Just(n)` or `None`) — case on it; there is no error/exception.\n"
    "- Map builtins (key argument FIRST): map_empty map_put map_get map_del map_size map_keys. "
