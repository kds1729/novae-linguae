...
- **Agent-authored algebraic properties** — `nl-validator check-properties` evaluates a record's `properties[]` against its worked `examples[]` three-valued (CONTRADICTED / UNVERIFIABLE / CONSISTENT — composition/functor laws are honestly marked unverifiable, not passed), and an opt-in `--properties` flag attaches a curated [law catalog](tooling/ingest-common/property_catalog.json) to recognised functions, verified against the examples on attach
