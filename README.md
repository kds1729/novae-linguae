...
- **Deeper ingestion + a richer runtime (v0.3+)** — lift the Haskell/TypeScript adapters' bodies beyond their conservative recognizers (the Python translator now covers conditionals / bindings / comprehensions / accumulator loops); scope-aware effect analysis that distinguishes an effect-polymorphic parameter applied directly from a genuinely external callee (`fn_ref` callees already resolve via `--records`); and a driver that orchestrates a multi-step `query → propose → commit → assert` conversation autonomously
