...
What is next:
- **Verification beyond *bounded* checks** — full quantified proof over unbounded domains (an SMT / inductive backend) and machine-checkable proof certificates. Today's property checking is example-bound, sampled (generative), or bounded-exhaustive; the next rung is a real proof obligation discharged by an SMT solver or an inductive prover, emitting a certificate a receiver can re-check rather than re-run
