...
- **Ingestion, remaining fidelity** — Rust `assert!` preconditions (extend predicate refinements to the second adapter that has asserts); **effect & termination inference** (records still carry `effects: []` and `terminates: "unknown"`); **body-expression ASTs** (the body is still a synthetic source hash, not a Nova Lingua AST — v0.3+); and full-AST parsers (`haskell-src-exts`, the TypeScript compiler) with richer TS types to replace the lightweight string scanners
...
- **Bootstrap-channel breadth & predicate properties** — additional/native bootstrap transports beyond the reference readers, and agent-authored algebraic *properties* (the schema supports them; ingestion only derives refinement preconditions, not laws)
