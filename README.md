...
- **Deeper ingestion + a richer runtime (v0.3+)** — full control-flow body ASTs so the *ingested* corpus is executable (today only single-expression bodies get a resolvable AST); and *static* effect inference (proving a body's effects ⊆ its declaration without running it) beyond today's syntactic ingestion lower bound and runtime enforcement
