...
What is next:
- **An even fuller agent loop** — the responder answers `apply`, `validate`, `query`, `propose`, the `store` action (verify payload → `ack`/`reject`), a received `commit` (fulfil the `apply` commitment → `assert` the result), and `delegate`/`retract` (acknowledged); still ahead: a driver that *orchestrates* a multi-step `query → propose → commit → assert` conversation autonomously, and effect/capability gating of what a delegation authorizes
