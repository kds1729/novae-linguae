...
- **Deeper ingestion + a richer runtime (v0.3+)** — extend the executable body subset further (loops, comprehensions; the Haskell/TypeScript adapters' bodies beyond their conservative recognizers); and *static* effect inference (proving a body's effects ⊆ its declaration without running it) beyond today's syntactic ingestion lower bound and runtime enforcement
