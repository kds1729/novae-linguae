...
- **Deeper ingestion + a richer runtime (v0.3+)** — the remaining effect kinds need real fs/net/process primitives in the evaluator (the four current ones cover `io.console`/`random`/`time`/`panic`); a real delegation-chain verifier behind the capability gate; and verification beyond *bounded* checks — full quantified proof over unbounded domains (an SMT / inductive backend) and machine-checkable proof certificates (today: example-bound, sampled, or bounded-exhaustive)
