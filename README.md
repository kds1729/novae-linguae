...
What is next:
- **Nova Locutio becomes actionable** — messages are validated, signed, encrypted, stored, and discoverable, but nothing yet *consumes* them to drive behavior. A reference responder (consume a signed `request` → resolve + run the target → emit a signed `assert` with the result) and a v0.2 request shape (value-expression args / `fn_ref`s) would close the agent conversation loop
- **Deeper ingestion + a richer runtime (v0.3+)** — full control-flow body ASTs so the *ingested* corpus is executable (today only single-expression bodies get a resolvable AST); effect inference beyond the syntactic lower bound; effect *enforcement* (a capability sandbox, not just descriptive metadata); and a generative property-testing engine (the rung above example-bound CONSISTENT)
