...
What is next:
- **Commons node, production hardening** — the neural embedder, pgvector ANN search, and the Postgres/Redis backends have landed (above). Remaining: a production deployment layer (`Dockerfile` + `docker-compose.prod.yml` — gunicorn web, a Celery worker for async embed/verify/replicate, a Caddy/nginx proxy with TLS/gzip/rate-limits), an **egress-budget governor** (a hard cap on the bill), Postgres GIN-indexed in-database typed query, and a replication worker (mirror peers)
- Higher-fidelity ingestion — **done for all four adapters** via a `--v2` mode: each emits real v0.2 function records with a **structured type-expression AST** (built from the language's types, reusing the shared `ingest-common/nl_types.py` builder; unknowns become fresh `forall`-bound type variables) and **real examples** mined from the language's in-source examples (Python/Haskell doctests, Rust `///` doc-tests, TypeScript JSDoc `@example`), encoded as value ASTs (`nl_values.py`) — falling back to v0.1 when a function has no usable examples. Canonical float serialization (JCS/ECMAScript) is pinned by conformance vectors. Still open: predicate ASTs for refinements/properties, body ASTs (v0.3+), and full-AST parsers (`haskell-src-exts` / the TypeScript compiler) to replace the lightweight string scanners
