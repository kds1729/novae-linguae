...
- **Deeper ingestion + a richer runtime (v0.3+)** — full control-flow body ASTs so the *ingested* corpus is executable (today only single-expression bodies get a resolvable AST); effect inference beyond the syntactic lower bound; and effect *enforcement* (a capability sandbox, not just descriptive metadata)
