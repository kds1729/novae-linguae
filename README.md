...
The "what's next" items from the prior milestones are now done (the `alloc` effect, a TLS + de-chunking net client, the delegation-chain verifier, the first-order SMT backend, the inductive backend, lemma discovery for helper-lemma inductions, and the trust-model policy engine + attestation graph). Remaining frontier: **theory exploration** — discovering lemmas *not* in the curated catalog by conjecturing and testing candidate equations over the goal's operations (the catalog is already the seam for it); `foldl`/`foldr` and induction over user-defined recursive bodies (`self`); richer trust policy (recency decay, confidence-weighting, full vertex-disjoint path diversity); and deeper v0.3+ ingestion fidelity.
