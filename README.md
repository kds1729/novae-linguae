...
What is next:
- **An even fuller agent loop** — the responder now answers `apply` (assert a result), `validate` (assert `verified` / `reject`), and `query` (`ack` matches); still ahead: the `store` action, the negotiating speech acts (propose / commit / delegate / retract), and multi-step conversations that chain query → compose → commit into autonomous work
- **Deeper ingestion + a richer runtime (v0.3+)** — extend the executable body subset further (loops, comprehensions; the Haskell/TypeScript adapters' bodies beyond their conservative recognizers); and resolve `fn_ref` callees during effect inference so a composed body verifies instead of reading UNVERIFIABLE
