...
- **Semantic equivalence vs hash equivalence.** Two functions can be hash-different but behaviorally identical. Need clustering, canonical forms, equivalence proofs. *(Partly addressed: `nl-validator equiv` / the node's `POST /v0/equiv` now **prove** `∀x. f(x) = g(x)` over the unbounded domain — the equivalence-proof piece — by reusing the SMT + induction + lemma-discovery prover. Clustering/canonical forms on top of it remain open.)*
