...
- **Deeper ingestion + a richer runtime (v0.3+)** — the remaining effect kinds (`net.read`/`net.write`/`process.spawn`) follow the same record/replay `effect_op` pattern as the real `fs.read`/`fs.write`, but network egress and process spawn are outward-facing surfaces deferred to a per-deployment opt-in; a real delegation-chain verifier behind the capability gate; and verification beyond *bounded* checks — full quantified proof over unbounded domains (an SMT / inductive backend) and machine-checkable proof certificates (today: example-bound, sampled, or bounded-exhaustive)
