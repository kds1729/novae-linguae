...
What is next:
- **A fuller agent loop** — the responder handles the `apply` request action with `predicate` claims; the remaining speech acts (propose/commit/delegate/…), the `store`/`validate` actions, and multi-step conversations (an agent that *queries* the commons, *composes* a plan, and *commits*) build on the same substrate
