...
What is next:
- **Commons reference node** — implement [`spec/commons.md`](spec/commons.md): a Django service over Postgres (durable records + `pgvector` semantic search) and Redis (hot cache, ephemeral message delivery, replication signalling). The protocol is specified; the node is the next build
