...
- **Effect enforcement — declared effects are now a capability the runtime checks** ([`spec/evaluation.md`](spec/evaluation.md)). The evaluator runs against a *granted* effect set: the two effectful builtins (`print` → `io.console`, `rand` → `random`) gate on it, and each performed effect is appended to a structured, AI-ingestible **trace** (principle 9). `run <record>` grants exactly the record's declared `signature.effects`, so a record that **under-declares** its effects fails its own examples (effects stop being mere metadata); `eval <body> --grant <effect>…` runs a standalone body and rejects any ungranted effect. `rand` draws from a fixed-seeded PRNG, so an effectful run is as replayable as a pure one and the trace *is* the replay log (principle 5). Worked example: [`greet.v0.2.json`](spec/examples/greet.v0.2.json) (`\msg -> print(msg)`, declaring `io.console`) runs clean, but is rejected under `eval` without the grant
...
- **Deeper ingestion + a richer runtime (v0.3+)** — emit runnable bodies from the Haskell/TypeScript adapters (parity with Python's `--emit-dir`) so their now-executable ASTs run end to end; the remaining effect kinds beyond the two effectful builtins, and effect/capability *gating* of what a delegation authorizes; and verification beyond example- and sampling-bound checks (a generative property engine that quantifies, proof certificates)
