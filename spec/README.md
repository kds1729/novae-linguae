...
|------|--------|------|
| `function-record.schema.json` | v0.1 **frozen** | The function-record schema, v0.1 (string surface form for type / predicate / value fields). Superseded by v0.2 for authoring new records; existing v0.1 records remain valid and resolvable forever (content-addressed — nothing breaks, nothing is force-migrated) |
| `function-record.v0.2.schema.json` | v0.2 draft | The function-record schema, v0.2 — same shape as v0.1 but with structured type / predicate / value ASTs (sub-schemas inlined under `$defs` for self-containment). Carries an optional v0.3 `signature.cost` (`time` / `measure` / `output_size`) enabling **precise** complexity composition through pipelines. An example's expected value may be carried **by address** (`examples[].result_blob` — sha256 + byte count of the JCS-canonical value-expression bytes, served from the gate-free `/v0/blobs/{sha256}` store) when it is too large to inline, e.g. a multi-MB observed document |
| `message.schema.json` | v0.1 **frozen** | The structured speech-act envelope for *Nova Locutio* messages. Superseded by v0.2 for authoring new messages; existing v0.1 messages remain valid and resolvable forever (content-addressed — nothing breaks, nothing is force-migrated) |
...
| `predicate-expression.schema.json` | v0.1 draft | Structured AST for refinement predicates, property tests, and assert claims (used inline by `function-record.v0.2.schema.json`). Includes `lit_blob` — a value literal **by address** (sha256 of its canonical value-expression bytes) so an assert claim about a multi-MB observed result stays publishable; verified by recompute-and-hash-compare, no blob travels |
| `value-expression.schema.json` | v0.1 draft | Structured AST for values in `examples.args` / `examples.result` (used inline by `function-record.v0.2.schema.json`). Thirteen kinds incl. the v0.4 **`map`** (`Map string a` values: unique keys sorted in code-point order — the canonical form the well-formedness checker enforces; surface `map {"k" => v, …}`) |
| `body-expression.schema.json` | v0.1 draft | Structured AST for the executable body that a function record's `body_hash` points to (nine expression kinds, five pattern kinds) |
| `claim-expression.schema.json` | v0.1 draft | Structured AST for `assert.claim` (predicate / **observed** — a predicate conditioned on a recorded effect trace by `trc_…` address, replay-verifiable without grants or secrets / satisfies / verified / **attestation** — a trust verb about an agent or artifact: `vouches-for` / `trusts-claims-about` / `distrusts` / **equivalent** — an extensional-equivalence claim between two functions, objective: any receiver re-proves it from the two bodies via `verify-claim`) |
| `trace.schema.json` | v0.1 draft | A recorded **effect trace** (`trc_<hex>`, hashless and self-addressing like a body expression): the ordered `{effect, detail, result}` observations of one effectful run. Secrets stay in `{{secret:NAME}}` placeholder form, so a trace is publishable and replay needs no credentials. Referenced by an `observed` claim; a verifier replays the claim's computation against it (strict order, exact consumption) |
| `plan.schema.json` | v0.1 draft | A **plan** as a commons artifact (`plan_<hex>`, strips `hash`; unsigned — soundness is recomputable, never testimony): the `{assume, steps}` that `check-plan` decides, content-addressed so a checked plan is fetchable, re-decidable, and citable by address (`check-plan --plan plan_… --node`). See `world-state.md` |
...
| `trust-model.md` | v0.1 | Normative spec for the trust model: local trust policy + capability tokens + attestations, no central authority. All three reference pieces now built in `nl-validator`: the capability verifier (`verify-delegation`), the attestation-graph query layer (`AttestationGraph` — which also ingests signed **certification** records as `certifies` edges), and the policy engine (`evaluate-trust` / `authorize` / `certified` — is a function certified by a certifier the policy trusts?). |
| `world-state.md` | v0.1 | **World-state refinements + plan checking** (evolution/gcp-sdk-poc open question 4): `requires`/`ensures` contracts over abstract external resources (a `class` keyed by the call's parameters — "requires a VPC, guarantees a subnet"; state vocabulary deliberately just `exists`/`absent`), carried in `signature.refinements[]` alongside value refinements (`$defs/world_refinement` in the v0.2 record schema). `nl-validator check-plan` symbolically executes a plan — a sequence of concrete applications — against the declarations, so a multi-step effectful plan is checked **before any effect**: `PLAN-SOUND` / `UNVERIFIABLE` (a requirement nothing establishes, never a silent pass) / `REJECTED` (a contradiction, e.g. use-after-delete). Honest grading: verifies the plan against the declarations, never the declarations against the world — those stay the author's testimony. Worked examples: `put-note` (the dependency shape over the fake service) + `plan-note-lifecycle` / `plan-note-on-deleted`. Plans are commons artifacts (`plan.schema.json`), and a plan's assumptions are spot-verifiable by **observation probes** (`check-plan --probe class=fn_…` — 2xx = exists, 404 = absent, a refuted assumption fails the check) |
| `intent-tag-vocabulary.md` | v0.1 | Controlled vocabulary for `intent_tags`: 18 top-level categories (`transform`, `predicate`, `aggregate`, `filter`, `query`, `parse`, `serialize`, `io`, `arithmetic`, `math`, `logical`, `string`, `concurrent`, `crypto`, `time`, `dispatch`, `coll`, and the **derived** `composite` axis — assembler-derived `uses`/`yields` tags, never hand-invented) plus property-modifier tags (`pure`, `elementwise`, `idempotent`, …). Non-vocab tags still validate; cross-agent agreement is the benefit. |
...
| `expressiveness.md` | v0.4 — phases 1–3 DONE | The v0.4 expressiveness plan and its execution log — from proof of concept to practical functionality: golden workflows (fetch→extract→transform, compute→format→report, dispatch-on-text) pull the primitives, and every addition pays the "verified-by-default tax" across the whole semantic core. **Phase 1 strings, phase 2 maps, and phase 3 JSON-as-data are live** (all with certified exit-gate records in `examples/`), the string domain has the full verification tier (SMT string fragment, generative string testing, concat normal form), and phase-4 follow-through (corpus families #39–41, Python/TS string-idiom ingestion) is under way. Totality via `Maybe` (never `error`), determinism throughout. |
| `resilience.md` | design | Availability/anti-sabotage design for the public service **Arca**: the "transport is untrusted" property, seed bundles, the standard `.nlb` commons-bundle release-artifact format, and a pluggable censorship-resistant bootstrap (HTTPS · IPNS · DNS-over-HTTPS · Nostr · chain-anchor · Tor · mirror). The mechanisms are **implemented** (deterministic `.nlb` format with signed manifests, node `exportbundle`/`loadbundle`, and the multi-channel `makebootstrap`/`bootstrap` resolver — see the layer-by-layer status table); the doc records the strategy and is not itself a normative protocol spec (`commons.md` is). Deployment + cost-control counterpart at [`../tooling/commons-node/DEPLOYMENT.md`](../tooling/commons-node/DEPLOYMENT.md). |
| `bundle.schema.json` | v0.1 draft | The `manifest.json` of a `.nlb` commons bundle (`nlb/1`): `{format_version, count, schema_versions[], bundle_digest, source?, producer?, signature?}`. `bundle_digest` (BLAKE2b-256 over the sorted record-hash set) is a whole-payload integrity pre-check, not the security boundary — a node re-verifies every record by hash on ingest, so the manifest (and its optional Ed25519 provenance signature) is advisory, never a trust gate (principle 7). Normative counterpart to the `.nlb` section of [`commons.md`](commons.md); implemented in [`../tooling/commons-node/commons/bundle.py`](../tooling/commons-node/commons/bundle.py). |
| `weights.md` | v0.1 | **Model weights in the commons**: a small content-addressed **pointer record** (`wgt_…` — base model, blob manifest with per-file sha256, the deterministic training `recipe` over content-addressed corpus data, advisory `urls[]`) while the blobs themselves live on ordinary static hosting (`GET /v0/blobs/{sha256}`, host untrusted — the hash is the truth). Verification in three rungs: integrity (hash the download), reproducibility (re-run the deterministic recipe), and signed **eval attestations** (the weights analogue of a function certification — a certifier's measured held-out score, ingested by the attestation graph as `attests-eval` edges so a policy can answer "attested by a certifier I trust?" and discovery can rank weights by *trusted* capability). Implemented end to end in `nl-validator` (`attest-weights`, `certified --subject wgt_…`) and the node (gate + `/attestations` + `/v0/blobs`); the pinned reference adapters are the first hosted weights. |
| `weights.schema.json` | v0.1 draft | The weights **pointer record** (`wgt_<hex>`, unsigned, hashed like a function record): `base` (model + license), `format` (closed enum), `files[]` (per-blob `{name, sha256, bytes}` — sha256 by ML-ecosystem convention; blobs never enter the JCS pipeline), the deterministic `recipe` (content-addressed corpus + split, trainer, seed, epochs, hyperparameters), optional *normatively-untrusted* self-reported `measured`, advisory `urls[]`, `derived_from`/`supersedes` lineage. |
| `eval-attestation.schema.json` | v0.1 draft | Signed **eval attestation** (`evl_<hex>`, produced by `attest-weights --sign`) — a certifier's measured-capability statement about a `wgt_…` subject: the `eval` descriptor (harness, settings, task-set content identity) + `results`, Ed25519-signed by the same canonical rules as a message. The trust layer ingests it as an `attests-eval` edge — for opaque bytes, the attestation layer is the artifact's value. |
...
| `examples/json-port.v0.2.json` | example | **JSON-as-data** v0.2 record (`spec/expressiveness.md` phase 3 — GW1's practical form): `json_port : string → int` — `parse_json` the payload, `case` on `Just(JObj(m))`, `map_get "port"`, `case` on `Just(JNum(p))`; every failure path (garbage, missing field, mistyped field) falls to the default, totally. Body [`body-json-port.json`](examples/body-json-port.json); **certifies fully SOUND** |
| `examples/config-port.v0.2.json` | example | **Map-consuming** v0.2 record (`spec/expressiveness.md` phase 2, the GW3 config-lookup idiom): `config_port : Map string int → int` — `case map_get "port" m of { Just(p) => p; None => int(8080) }`. Examples carry real `map` values (incl. the empty map); body [`body-config-port.json`](examples/body-config-port.json); **certifies fully SOUND**. `map_get`'s `Maybe` is the same totality pattern as `parse_int` |
| `examples/json-get.v0.2.json`, `examples/json-path.v0.2.json` | example | **Nested-JSON projection** as ordinary certified commons functions, not builtins: `json_get : string → Json → Maybe Json` (one `case` on `JObj` + `map_get`) and `json_path : List string → Json → Maybe Json` (structural recursion over the key path — empty path `Just`, any miss `None`). The first signatures to *mention* the nominal `Json` builtin type; bodies [`body-json-get.json`](examples/body-json-get.json)/[`body-json-path.json`](examples/body-json-path.json); both **certify fully SOUND** (`O(n)` / `O(n²)`) |
| `examples/weights-*.json`, `examples/attest-*.json` | example | The **first hosted weights** ([`weights.md`](weights.md)): `wgt_` pointer records for the pinned reference adapters (Coder-3B/7B on corpus12, Coder-14B on corpus10 — base, blob manifest, deterministic recipe, advisory Arca `/v0/blobs` urls) and the certifier's signed `evl_` eval attestations of their measured 360-task held-out scores (`attest-weights --sign`, byte-reproducible — no timestamp) |
| `examples/insert-sorted.v0.2.json`, `examples/sort-strings.v0.2.json`, `examples/logins-of.v0.2.json`, `examples/contributors-report.v0.2.json`, `examples/fetch-contributors-report.v0.2.json` | example | **GW4, the sorted report** — the workflow that pulled the tier-2 builtins `str_lt`/`str_lower` ([`expressiveness.md`](expressiveness.md)). Sorting is in-language: `insert_sorted` (the one recursive piece) folded by `sort_strings` via `fn_ref` (*assemble, don't write*); `logins_of` projects a Json list; `contributors_report` is the pure parse→project→lower→sort→join pipeline; `fetch_contributors_report` is the effectful leg (declared `net.read`, fulfilled responder-side only under an operator `--grant`). All five certify |
| `examples/csv-second-int.v0.2.json` | example | **String-manipulating** v0.2 record (`spec/expressiveness.md` phase 1, GW1): `csv_second_int : string → Maybe int` — split a textual payload, walk to the second field, `parse_int` it. Body [`body-csv-second-int.json`](examples/body-csv-second-int.json); **certifies fully SOUND** (typecheck, effects, termination, `O(n)` complexity). Totality via `Maybe`: a malformed field is `None`, never an error |
| `examples/double-second-field.v0.2.json` | example | GW1's transform step: `double_second_field : string → int` — the `csv_second_int` shape with the `Maybe` destructured in a `case` (`Just(n) => n + n; None => 0`). Body [`body-double-second-field.json`](examples/body-double-second-field.json); certifies fully SOUND. Discovered by intent `parse` and applied via `orchestrate --verify --require-certified` (CONFIRMED, 42) |
| `examples/render-csv.v0.2.json` | example | GW2's format step: `render_csv : List int → string` = `str_join "," (map to_string xs)`. Body [`body-render-csv.json`](examples/body-render-csv.json); certified (termination/complexity honestly UNVERIFIABLE — `map` is higher-order) |
...
| `examples/type-builtin-<name>.json` (×14) | canonical | The canonical commons type artifact for each builtin (the v0.2 builtin→ref fold): the builtin node itself plus its `type_…` hash — a `ref` to one is interchangeable with the builtin spelling, folded locally by address recompute (`nl-validator canonical-types` regenerates) |
...
# Is a FUNCTION certified by a certifier the policy trusts? Ingests signed certification records (certifies
# edges) alongside vouch attestations, then checks that a certifier of the function is itself trusted.
./tooling/validator/target/release/nl-validator certified --policy spec/examples/trust-policy.json \
  --attestations spec/examples/attestations --subject <fn-hash>
```
