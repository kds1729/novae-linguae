...
| `weights.md` | proposed design | **Model weights in the commons**: a small content-addressed **pointer record** (`wgt_…` — base model, blob manifest with per-file sha256, the deterministic training `recipe` over content-addressed corpus data, advisory `urls[]`) while the blobs themselves live on ordinary static hosting (`GET /blobs/<sha256>`, host untrusted — the hash is the truth). Verification in three rungs: integrity (hash the download), reproducibility (re-run the deterministic recipe), and signed **eval attestations** (the weights analogue of a function certification — a certifier's measured held-out score, ingested by the attestation graph so discovery can rank weights by *trusted* capability). Not yet implemented — no `wgt_` kind exists in the validator or node yet. |
