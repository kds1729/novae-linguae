...
| `expressiveness.md` | plan | The v0.4 expressiveness plan — from proof of concept to practical functionality: golden workflows (fetch→extract→transform, compute→format→report, dispatch-on-text), the workflow-pulled minimal primitive sets (phase 1 **strings**, phase 2 **maps**, phase 3 **JSON-as-data**), the "verified-by-default tax" every addition must pay across the whole semantic core, and explicit non-goals. Totality via `Maybe` (never `error`), determinism throughout. |
