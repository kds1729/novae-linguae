...
| `world-state.md` | v0.1 | **World-state refinements + plan checking** (evolution/gcp-sdk-poc open question 4): `requires`/`ensures` contracts over abstract external resources (a `class` keyed by the call's parameters — "requires a VPC, guarantees a subnet"; state vocabulary deliberately just `exists`/`absent`), carried in `signature.refinements[]` alongside value refinements (`$defs/world_refinement` in the v0.2 record schema). `nl-validator check-plan` symbolically executes a plan — a sequence of concrete applications — against the declarations, so a multi-step effectful plan is checked **before any effect**: `PLAN-SOUND` / `UNVERIFIABLE` (a requirement nothing establishes, never a silent pass) / `REJECTED` (a contradiction, e.g. use-after-delete). Honest grading: verifies the plan against the declarations, never the declarations against the world — those stay the author's testimony. Worked examples: `put-note` (the dependency shape over the fake service) + `plan-note-lifecycle` / `plan-note-on-deleted`. |
